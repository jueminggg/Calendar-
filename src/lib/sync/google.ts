...
import { clientForConnection, fetchGoogleEvents } from "@/lib/providers/google";
import { syncCalendarEvents } from "./upsert";

export async function syncGoogleConnection(connectionId: string, isInitialSync = false) {
  ...
  try {
    for (const cal of connection.calendars) {
      const result = await fetchGoogleEvents(client, cal.externalId, cal.syncCursor);
